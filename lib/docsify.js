(function() {
		...
		var aside =
			...
			(config.name
				? '<h1 class="app-name"><a class="app-name-link" data-nosearch>' +
				  (config.logo ? '<img alt=' + config.name + ' src=' + config.logo + '>' : config.name) +
				  '</a></h1>'
				: '') +
				...
				while (src) {
					...
					if ((cap = this$1.rules.em.exec(src))) {
						...
						out += this$1.renderer.em(
							this$1.output(cap[6] || cap[5] || cap[4] || cap[3] || cap[2] || cap[1])
						);
						continue;
					}
				...
				switch (this.token.type) {
					...
					case 'list_item_start': {
						...
						while (this.next().type !== 'list_item_end') {
							body += !loose && this$1.token.type === 'text' ? this$1.parseText() : this$1.tok();
						}
    ...
    ********************************************** */
	...
	function formatUpdated(html, updated, fn) {
		updated =
			typeof fn === 'function' ? fn(updated) : typeof fn === 'string' ? tinydate(fn)(new Date(updated)) : updated;
		...
		proto._bindEventOnRendered = function(activeEl) {
			...
			if (autoHeader && activeEl) {
				...
				if (firstNode && firstNode.tagName !== 'H1') {
					var pageHeader = create('div');
					pageHeader.classList.add('page-header');
					var h1 = create('h1');
					h1.innerText = activeEl.innerText;
					appendTo(pageHeader, h1);
					before(main$$1, pageHeader);
				}
			}
	...
	function getAlias(path, alias, last) {
		...
		return match ? getAlias(path.replace(cached$2[match], alias[match]), alias, path) : path;
	}
	...
	History.prototype.toURL = function toURL(path, params, currentRoute) {
		...
		if (local) {
			...
			path = (idIndex > 0 ? currentRoute.substr(0, idIndex) : currentRoute) + path;
		}
